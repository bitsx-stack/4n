...
const StockTakingPage: React.FC = () => {
  ...
  const [selectedStorage, setSelectedStorage] = useState<OptionType | null>(
    null,
  );
  ...
  // Fetch brands: get all category types, find the one named "Brand", then load its categories
  useEffect(() => {
    api
      .get("/category-types", { params: { pageSize: 1000 } })
      .then((res) => {
        const types = res.data.data || res.data || [];
        const brandType = types.find(
          (t: any) => t.name.toLowerCase() === "brand",
        );
        if (!brandType) return;
        return api.get(`/categories/type/${brandType.id}`, {
      ...
      .then((res) => {
        ...
        setBrands(all.map((b: any) => ({ value: b.id, label: b.name })));
      });
  }, []);

  // Fetch models when brand changes: use selected brand name as category type name
  useEffect(() => {
    if (!selectedBrand) {
      setModels([]);
      return;
    ...
    api
      .get(`/categories/typename/${selectedBrand.label}`, {
        params: { pageSize: 2000 },
      })
      .then((res) => {
        const all = res.data.data || res.data || [];
        setModels(all.map((m: any) => ({ value: m.id, label: m.name })));
      });
  }, [selectedBrand]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post("/imeis", {
        code: barcode,
        brand: selectedBrand?.label, // Send brand name, not ID
        model: selectedModel?.label, // Send model name, not ID
    ...
    <AdminDashboardLayout>
      <DashboardCard title="Stock Taking">
        <form onSubmit={handleSubmit} className="tw-space-y-4">
          ...
          {message && (
            <div className="tw-mt-2 tw-text-center tw-text-primary">
              {message}
            </div>
          )}
        </form>
